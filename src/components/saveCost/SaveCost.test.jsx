...
test('should render dialog without item', () => {
  ...
  expect(
    getByText(trans('Components.SaveCost.createPrice')),
  ).toBeInTheDocument();
});
...
test('should render dialog with item', () => {
  const testItem = {
    name: 'Administración',
    comment:
      'Dormouse, without considering at all this grand procession, came THE KING AND QUEEN OF HEARTS. Alice was soon submitted to by.',
    price: '1251.22',
  };
  ...
  expect(
    getByText(trans('Components.SaveCost.updatePrice')),
  ).toBeInTheDocument();
